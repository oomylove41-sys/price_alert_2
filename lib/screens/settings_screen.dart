...
// Exports 6 public settings page widgets used by MainShell.
      ...
      child: Column(
        ...
        children: [
          _InfoBanner(
            ...
            text: 'Set how often the bot checks Binance for new HH/LL levels and candlestick patterns.',
          ),
          const SizedBox(height: 20),
...
// ══════════════════════════════════════════════════════════
// ─── PAGE 6: PATTERN SETTINGS ────────────────────────────
// ══════════════════════════════════════════════════════════
class PatternSettingsPage extends StatefulWidget {
  final VoidCallback onSaved;
  const PatternSettingsPage({super.key, required this.onSaved});

  @override
  State<PatternSettingsPage> createState() => _PatternSettingsPageState();
}

class _PatternSettingsPageState extends State<PatternSettingsPage> {
  late bool   _patternBE;
  late bool   _patternMS;
  late bool   _patternES;
  late bool   _requireTrend;
  late double _starBodyPct;
  late double _recoveryPct;
  bool _saving = false;

  @override
  void initState() {
    super.initState();
    _patternBE    = Config.patternBE;
    _patternMS    = Config.patternMS;
    _patternES    = Config.patternES;
    _requireTrend = Config.patternRequireTrend;
    _starBodyPct  = Config.patternStarBodyPct;
    _recoveryPct  = Config.patternRecoveryPct;
  }

  Future<void> _save() async {
    setState(() => _saving = true);
    await ConfigService.savePatternSettings(
      patternBE:          _patternBE,
      patternMS:          _patternMS,
      patternES:          _patternES,
      patternRequireTrend: _requireTrend,
      patternStarBodyPct:  _starBodyPct,
      patternRecoveryPct:  _recoveryPct,
    );
    setState(() => _saving = false);
    widget.onSaved();
  }
  ...
  Widget _buildSliderTheme(BuildContext context, bool isDark, Widget child) {
    return SliderTheme(
      data: SliderTheme.of(context).copyWith(
        activeTrackColor:   Colors.blueAccent,
      ...
      child: Column(
        ...
        children: [
          _InfoBanner(
            ...
            text:
                'Detect Bullish Engulfing, Morning Star & Evening Star patterns '
                'and receive instant Telegram alerts.',
          ),
          const SizedBox(height: 20),

          // ─── Pattern toggles ────────────────────────
          _FieldLabel('Active Patterns'),
          const SizedBox(height: 8),

          _buildToggleRow(
            title:     '🟢  Bullish Engulfing (BE)',
            subtitle:  'C1 bearish fully engulfed wick-to-wick by C2 bullish',
            value:     _patternBE,
            onChanged: (v) => setState(() => _patternBE = v),
            isDark:    isDark,
          ),
          _buildToggleRow(
            title:     '☀️  Morning Star (MS)',
            subtitle:  'Large bearish → doji/star → bullish recovery',
            value:     _patternMS,
            onChanged: (v) => setState(() => _patternMS = v),
            isDark:    isDark,
          ),
          _buildToggleRow(
            title:     '🌙  Evening Star (ES)',
            subtitle:  'Large bullish → doji/star → bearish reversal',
            value:     _patternES,
            onChanged: (v) => setState(() => _patternES = v),
            isDark:    isDark,
          ),
          ...
          _buildToggleRow(
            title:     'Require Prior Trend',
            subtitle:  'Only alert after 3 confirmed bars trending in the right direction',
            value:     _requireTrend,
            onChanged: (v) => setState(() => _requireTrend = v),
            isDark:    isDark,
          ),
          ...
          // ─── Star body % slider ──────────────────────
          _FieldLabel('Star Max Body  —  ${_starBodyPct.toStringAsFixed(0)}%'),
          const SizedBox(height: 4),
          Text(
            'A candle is a "star" when its body is ≤ this % of its high-low range.',
            style: TextStyle(fontSize: 12, color: Colors.grey.shade500),
          ),
          ...
          _buildSliderTheme(
            ...
            Slider(
              ...
              max:       50,
              divisions: 9,
              onChanged: (v) => setState(() => _starBodyPct = (v / 5).round() * 5.0),
            ),
          ),
          Row(
            mainAxisAlignment: MainAxisAlignment.spaceBetween,
            children: const [
              Text('5%',  style: TextStyle(fontSize: 11, color: Colors.grey)),
          ...
          // ─── Recovery % slider ──────────────────────
          _FieldLabel('MS / ES Recovery  —  ${_recoveryPct.toStringAsFixed(0)}%'),
          const SizedBox(height: 4),
          Text(
            'The closing candle must recover at least this % into the first candle\'s body.',
            style: TextStyle(fontSize: 12, color: Colors.grey.shade500),
          ),
          const SizedBox(height: 6),
          _buildSliderTheme(
            ...
            Slider(
              ...
              max:       80,
              divisions: 12,
              onChanged: (v) => setState(() => _recoveryPct = (v / 5).round() * 5.0),
            ),
          ),
          Row(
            mainAxisAlignment: MainAxisAlignment.spaceBetween,
            children: const [
              ...
              Text('80%', style: TextStyle(fontSize: 11, color: Colors.grey)),
            ],
          ),
